...
class LoadDataFromExcelTest(TestCase):
    ...
    def setUp(self):
        with open(TESTDATA_FILENAME, "rb") as infile:
            self.dataset = tablib.import_set(infile.read())
        ...
        self.raw = {
            ...
            "Procestypetoelichting": (
                "Dit procestype betreft het instellen van een nieuw organisatieonderdeel "
                "of een nieuw orgaan waar het orgaan in deelneemt. Dit procestype betreft "
                "eveneens het inrichten van het eigen orgaan. Dit kan kleinschalig "
                "plaatsvinden bijvoorbeeld het wijzigen van de uitvoering van een "
                "wettelijke taak of grootschalig wanneer er een organisatiewijziging "
                "wordt doorgevoerd."
            ),
    ...
    def test_prepare_resultaat_generiek(self):
        ...
        """
        proces_type = ProcesType.objects.create(**prepare_procestype(self.raw, self.jaar))
    ...
    def test_command_not_unique_procestype(self):
        ...
        proces_type = ProcesType.objects.create(**prepare_procestype(unique_data, self.jaar))
